...
void ProcessEventsMovments(){
    if(sendFlag ==0){
        ADC_SAR_Joy_IsEndConversion(ADC_SAR_Joy_WAIT_FOR_RESULT);
...
void ScanProgressEventHandler(CYBLE_GAPC_ADV_REPORT_T* eventParam)
{
    ...
    uint8 Macaa = memcmp(eventParam->peerBdAddr,(uint8*)0x984fee0f7ff8,6);
    printf("if Mac:%d", Macaa);
